# 突击莉莉角色识别项目
## 一、项目概述
本项目是一个基于pytorch的动漫角色识别系统，专门针对《突击莉莉》(Assault Lily) 系列作品中的角色进行识别。项目包含完整的模型训练功能。有使用vibecoding

本项目提供的训练好的模型所使用的游戏卡面数据截止至2026/5，对于新的卡面识别能力有限（训练的时候有点过拟合了），且我的显卡性能不太行，过于难以训练参数更大的模型，对于新卡面的识别仅能作为参考，但（由于过拟合）对就卡面的识别还是比较准的（悲
### 项目结构
```
突击莉莉角色识别/
├── core/
